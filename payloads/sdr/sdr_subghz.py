...
def _format_signal(sig):
    ...
    parts = []
    if sig.get("id"):
        parts.append(f"ID:{sig['id']}")
    if sig.get("channel"):
        parts.append(f"Ch:{sig['channel']}")
    if sig.get("temperature_C") is not None:
        parts.append(f"{sig['temperature_C']}°C")
    if sig.get("humidity") is not None:
        parts.append(f"{sig['humidity']}%")
    if sig.get("button") is not None:
        parts.append(f"Btn:{sig['button']}")
    if sig.get("code"):
        parts.append(f"Code:{sig['code']}")
    if sig.get("data"):
        parts.append(f"Data:{str(sig['data'])[:12]}")
    if sig.get("pressure_kPa") is not None:
        parts.append(f"{sig['pressure_kPa']}kPa")
        ...
        for line in _rtl_proc.stdout:
            ...
            try:
                ...
                data["_category"] = _categorize(
                    data.get("model", ""), data.get("protocol", "")
                )
                with _signal_lock:
...
def _draw_live(band_idx, scroll):
    ...
    draw.text((SX(2), H - SY(11)), "OK:Rec LR:Band K1:View K2:Save", font=font_xs, fill=(50, 60, 80))
